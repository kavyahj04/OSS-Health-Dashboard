...
function ScoreCircle({ score }: { score: number }) {
  ...
  return (
    <div className={`w-14 h-14 rounded-full border-2 flex items-center justify-center font-bold text-sm shrink-0 ${colorClass}`}>
      {score}
    </div>
  )
}
...
async function fetchRepos() {
    ...
    console.log("commitActivity sample:", data.repositories?.[0]?.commitActivity)
  ...
  const avgScore = reposWithScores.length > 0
    ? Math.round(
        reposWithScores.reduce((sum, r) => sum + r.healthScores[0].score, 0)
        / reposWithScores.length
      )
    : 0
  const healthyCount = reposWithScores.filter(r => r.healthScores[0].score >= 70).length
  const attentionCount = reposWithScores.filter(r => r.healthScores[0].score < 70).length

  // Aggregate commit activity across all repos for chart
  const commitChartData = (() => {
    const reposWithActivity = repos.filter(r => r.commitActivity && r.commitActivity.length > 0)
...
>
      {/* NAVBAR */}
      <nav className="bg-indigo-700 px-8 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            onClick={handleSync}
            disabled={syncing}
            className="bg-orange-500 hover:bg-orange-600 text-white px-5 py-2 rounded-lg font-semibold text-sm transition disabled:opacity-60 cursor-pointer"
          >
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-4 gap-4 mb-8">
          {[
            { label: "Total Repos",      value: repos.length,  color: "text-blue-600"   },
            { label: "Avg Health Score", value: avgScore,       color: "text-orange-500" },
            { label: "Healthy Repos",    value: healthyCount,   color: "text-green-600"  },
            { label: "Need Attention",   value: attentionCount, color: "text-red-500"    },
          ].map((stat, i) => (
            <div key={i} className="bg-white border border-blue-200 rounded-xl p-5 shadow-sm">
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-4 mt-3 pt-3 border-t border-blue-100">
                      ...
                      <span className="text-xs text.slate-500">👥 {repo._count.contributors}</span>
                      <span className="text-xs text-slate-500">📝 {repo.totalCommits}</span>
                      {repo.language && (
